...
void bfs_modificata()
{
  ...
  while (true)
  {
    ...
    for (int u : g[curr.node])
    {
      ...
      for (queue_elem v : q)
      {
        // se il nodo che vogliamo aggiungere e' gia' stato visitato, con la lunghezza che e' sempre pari / dispari, non serve rivisitarlo
        if (v.node == u && v.dist % 2 == new_dist % 2)
        {
          add_elem = false;
          break;
        }
      ...
      if (add_elem)
        q.push_back({u, new_dist, curr.node});
    }
  }
}
